...
import type { AgentCheckoutFormResponse, AgentLogisticsResponse } from '../../../types/agent';
...
export function CheckoutFormCard({ payload, onOpenOrder }: CheckoutFormCardProps) {
  ...
  useEffect(() => {
    ...
    const paidOrderId = paidOrder.order_id;
    let disposed = false;

    async function loadLogistics() {
      try {
        const payload = await paymentApi.getLogistics(paidOrderId);
        if (!disposed) {
    ...
    <section className={`payment-card${error ? ' payment-card--has-error' : ''}`}>
      ...
      {paidOrder ? (
        <>
          ...
          <article className="payment-track-inline">
            <div className="payment-track-inline__head">
              <strong>货单进度</strong>
              <p>{paidOrder.waybill_id || logistics?.waybill_id || '待生成运单号'}</p>
            </div>
