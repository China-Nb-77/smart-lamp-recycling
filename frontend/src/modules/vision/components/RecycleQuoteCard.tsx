...
export function RecycleQuoteCard({ payload }: RecycleQuoteCardProps) {
  ...
  const lampTypeLabel = payload.summary.lamp_type_label || installType;
  const lampTypeScore = payload.summary.lamp_type_score;
    ...
    <article className="biz-card biz-card--vision">
      <div className="biz-card__head">
        ...
        <div>
          ...
          <p>识别种类：{lampTypeLabel}</p>
          {typeof lampTypeScore === 'number' ? (
            <p>置信度：{(lampTypeScore * 100).toFixed(1)}%</p>
          ) : null}
        </div>
      </div>
