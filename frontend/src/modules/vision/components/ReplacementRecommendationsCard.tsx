...
const ADDRESS_CACHE_KEY = 'ai-light.shipping.addresses';
...
function loadAddressCache(): AddressEntry[] {
  try {
    const raw = window.localStorage.getItem(ADDRESS_CACHE_KEY);
    if (!raw) {
      return [];
    }
...
function saveAddressCache(addresses: AddressEntry[]) {
  window.localStorage.setItem(ADDRESS_CACHE_KEY, JSON.stringify(addresses));
}
...
export function ReplacementRecommendationsCard({
  ...
  useEffect(() => {
    ...
    async function bootstrapAddress() {
      const cached = loadAddressCache();
      if (cached.length > 0 && !cached[0].fullAddress.includes('请点击修改')) {
        setAddresses(cached);
        setSelectedAddressId(cached[0].id);
        setAddressForm((current) => ({
          ...current,
          fullAddress: cached[0].fullAddress,
          detailAddress: cached[0].detailAddress,
          contactName: cached[0].contactName,
          contactPhone: cached[0].contactPhone,
          tag: cached[0].tag,
      ...
      <div className="biz-card__head lamp-commerce__head">
        <div>
          ...
          <p className="lamp-commerce__intro">
            基于回收识别结果，已按
            {getSpaceLabel(payload.space || undefined) || '安装空间'}
            /
            {getBudgetLabel(payload.preferences.budget_level || undefined) || '预算均衡'}
            /
            {getInstallTypeLabel(payload.preferences.install_type || undefined) || '灯具'}
            为你推荐可替换灯具。
          </p>
        </div>
      </div>
          ...
          return (
            <section key={item.sku_id} className="lamp-commerce-card">
              ...
              <footer className="lamp-commerce-card__footer">
                ...
                <div className="lamp-commerce-card__actions">
                  <button
                    type="button"
                  ...
                  {item.buy_url ? (
                    <button
                      type="button"
                      className="secondary-button lamp-commerce-card__buy-btn"
                      onClick={() => window.open(item.buy_url || '', '_blank', 'noopener,noreferrer')}
                    >
                      去购买
                    </button>
                  ) : null}
