...
export type ChatFlowServerSession = {
  session_id: string;
...
export type ChatFlowSessionState = {
  server_session?: ChatFlowServerSession;
...
export function getServerSessionBinding(sessionId: string) {
  return getChatFlowSession(sessionId)?.server_session;
}

export function findServerSessionByRemoteId(remoteSessionId: string) {
  const store = loadStore();
  return Object.values(store).find(
    (session) => session.server_session?.session_id === remoteSessionId,
  )?.server_session;
}

export function findLocalSessionIdByRemoteId(remoteSessionId: string) {
  const store = loadStore();
  return (
